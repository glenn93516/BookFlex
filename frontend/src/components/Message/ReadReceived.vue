<template>
  ...
  <div>
    <div :class="size" class="readReceived-container">
      <!-- header -->
      <header>
        <div class="readReceived-header">
          <div class="readReceived-title">
            <img 
              class="mouse-pointer"
              width="25px"
      ...
      <!-- body -->
      <body class="readReceived-body">
        <hr class="my-hr2">

        <div class="readReceived-sender">
          <p>보낸 사람: {{ messageInfo.senderNickname }}</p>
          <p>보낸 일시: {{ messageInfo.receivemsgDate }}</p>
        </div>
        
        <div class="readReceived-content">
          {{ messageInfo.receivemsgContent }}
        </div>
      </body>
      ...
      <footer>
        <b-button 
          block
          class="mt-3 btn-warning readReceived-button"
          @click="goToWrite"
        >
        <img 
          width="20px" 
...
export default {
  ...
  methods: {
    getReceivedMessage(){
      ...
      this.$axios.get(`${this.$store.getters.getServer}/receivemsg/${this.$route.params.id}`,{headers})
      .then(res => {
    ...
    deleteReceivedMessage() {
      ...
      this.$axios.delete(`${this.$store.getters.getServer}/receivemsg/${this.$route.params.id}`,{headers})
      .then(() => {
        this.goToReceived()
      })
      .catch(err => {
