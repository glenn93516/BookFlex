<template>
  ...
  <div>
    <div :class="size" class="readSent-container">
      <!-- header -->
      <header>
        <div class="readSent-header">
          <div class="readSent-title">
            <img 
              class="mouse-pointer"
              width="25px"
      ...
      <!-- body -->
      <body class="readSent-body">
        <hr class="my-hr2">

        <div class="readSent-sender">
          <p>받은 사람: {{ messageInfo.receiverNickname }}</p>
          <p>받은 일시: {{ messageInfo.sendmsgDate }}</p>
        </div>
        
        <div class="readSent-content">
          {{ messageInfo.sendmsgContent }}
        </div>
      </body>
      ...
      <footer>
        <b-button 
          block
          class="mt-3 btn-warning readSent-button" 
          @click="goToWrite"
        >
          <img 
            width="20px" 
...
export default {
  ...
  methods: {
    getSentMessage(){
      ...
      this.$axios.get(`${this.$store.getters.getServer}/sendmsg/${this.$route.params.id}`,{headers})
      .then(res => {
    ...
    deleteSentMessage() {
      ...
      this.$axios.delete(`${this.$store.getters.getServer}/sendmsg/${this.$route.params.id}`,{headers})
      .then(() => {
        this.goToSent()
      })
      .catch(err => {
